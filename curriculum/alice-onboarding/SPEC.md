...
- Explain how entities communicate asynchronously across multiple transports (filesystem, Keybase, HTTP mesh, GitHub Issues)
...
The curriculum now has two layers before the numbered sights:

1. **Preamble** — friendship and articulation. Not a level. Alice meets the learner, establishes relationship, and practices articulation before content begins. (files: `preamble.md`)

2. **Belt 0** — three foundational concepts confirmed conversationally before sight 1. Not numbered levels. No `LEVEL_COMPLETE` signals. (files: `belt-0a.md`, `belt-0b.md`, `belt-0c.md`)
   - **0a: Folders Are Folders** — kingdoms are folders on disk; physical ownership
   - **0b: Git Is Folders with Undo** — worst case is always undo; removes fear of breaking things
   - **0c: Words Are Numbers but Easier to Type** — 24 seed words = keys; no server holds them

Then numbered levels 0–12 (the 13 sights).
...
| # | Title | Atoms | Minutes |
|---|-------|-------|---------|
| preamble | Friendship First, Articulation as the Door | — | 10–30 |
| 0a | Folders Are Folders | 1 | 5 |
| 0b | Git Is Folders with Undo | 1 | 5 |
| 0c | Words Are Numbers but Easier to Type | 1 | 8 |
...
| 9 | Async Messaging — How Entities Communicate | 4 | 20 |
...
| 1.3.2 | 2026-04-07 | Level 9 conceptual reframe — async messaging primitives, not GitHub-only (koad/chiron#4). Rewrote all four atoms: 9.1 Why Async (entities sleep, messages wait), 9.2 Message Anatomy (transport-agnostic sender/recipient/intent/completion), 9.3 Local First (comms/inbox/outbox/, .koad-io/ breadcrumbs as sovereign floor), 9.4 Networked Transports (Keybase primary, HTTP mesh, GitHub Issues for public coordination). Full dialogue, branching paths, delivery notes, and assessment revised. GitHub Issues repositioned from "the protocol" to "one transport, chosen for public visibility." Atom count unchanged (4). |
| 1.4.0 | 2026-04-16 | Pre-sight-1 belt-0 layer added. Three foundational concepts (0a/0b/0c) authored and inserted before sight 1. Preamble canonized in Chiron workspace (previously only existed in live delivery). chiron#7 (friendship preamble) and chiron#8 (articulation-to-orchestrator) reconciled into preamble.md. chiron#8 old Level 0 concept folded into Phase 2 of preamble — not a separate level. koad/alice#8 (three foundational concepts) delivered as belt-0a, belt-0b, belt-0c. Atom count: 60 → 63. Estimated hours: 4.5 → 4.75. |
| 1.4.1 | 2026-04-16 | Header reconciliation. `estimated_hours` corrected from 4.75 to 4.9 (293 min actual per-level sum after level-03/04 corrections). Flagged by Argus finding D5 in flight 95abfd1. |
